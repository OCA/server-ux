...
class TierValidation(models.AbstractModel):
    ...
    def _compute_next_review(self):
        for rec in self:
            ...
            rec.next_review = review and self.env._(
                "Next: %(review_name)s", review_name=review.name or ""
            )
                    ...
                    if not self._calc_reviews_validated(reviews):
                        ...
                        raise ValidationError(
                            self.env._(
                                ...
                                "- %(pending_reviews_str)s "
                                "\nPlease request a validation.",
                                pending_reviews_str=pending_reviews_str,
                            )
                        )
    ...
    def _notify_accepted_reviews_body(self):
        ...
        if has_comment:
            ...
            return self.env._("A review was accepted. (%(comment)s)", comment=comment)
    ...
    def _notify_rejected_review_body(self):
        ...
        return self.env._(
            "A review was rejected by %(user_name)s.", user_name=self.env.user.name
        )
    ...
    def _notify_created_review_body(self):
        return self.env._(
            "A record to be reviewed has been created by %(user_name)s.",
            user_name=self.env.user.name,
        )

    def _notify_requested_review_body(self):
        return self.env._(
            "A review has been requested by %(user_name)s.",
            user_name=self.env.user.name,
        )
    ...
    def _notify_restarted_review_body(self):
        return self.env._(
            "The review has been reset by %(user_name)s.", user_name=self.env.user.name
        )
