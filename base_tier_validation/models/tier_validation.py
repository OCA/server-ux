...
class TierValidation(models.AbstractModel):
    ...
    def validate_tier(self):
        ...
        if self.has_comment and any(option in ['all', 'validate'] for option in reviews.mapped('comment_option')):
    ...
    def reject_tier(self):
        ...
        sequences = self._get_sequences_to_approve(self.env.user)
        reviews = self.review_ids.filtered(lambda l: l.sequence in sequences)
        if self.has_comment and any(option in ['all', 'reject'] for option in reviews.mapped('comment_option')):
