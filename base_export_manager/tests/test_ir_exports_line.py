...
class TestIrExportsLineCase(TransactionCase):
    ...
    def test_check_name(self):
        ...
        m_ir_exports_line.create({"name": "name", "export_id": self.export.id})
        with self.assertRaises(ValidationError):
            m_ir_exports_line.create({"name": "name", "export_id": self.export.id})
        with self.assertRaises(ValidationError):
            m_ir_exports_line.create(
                {"name": "bad_error_name", "export_id": self.export.id}
            )

    def test_get_label_string(self):
        m_ir_exports_line = self.env["ir.exports.line"]
        export_line = m_ir_exports_line.create(
            {"name": "parent_id/name", "export_id": self.export.id}
        )
        self.assertEqual(
    ...
    def test_write_name_same_root(self):
        self.env["ir.exports.line"].create(
            {"export_id": self.export.id, "name": "parent_id"}
        )
        line = self.env["ir.exports.line"].create(
            {"export_id": self.export.id, "name": "name"}
        )
