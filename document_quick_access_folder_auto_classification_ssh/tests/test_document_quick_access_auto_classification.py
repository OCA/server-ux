...
PATCH_CLASS = (
    "odoo.addons.document_quick_access_folder_auto_classification_ssh.models."
    "document_quick_access_rule.SSHClient"
)
...
class TestDocumentQuickAccessClassification(TransactionCase):
    ...
    def test_ok_pdf_multi_limit(self):
        ...
        file = tools.file_open(
            'test_file.pdf',
            mode="rb",
            subdir="addons/document_quick_access_"
                   "folder_auto_classification_ssh/tests"
        ).read()
        self.env['document.quick.access.rule'].create({
            'model_id': self.model_id.id,
            'name': 'PARTNER',
        ...
        with patch(
            ...
            with patch(PATCH_CLASS) as mock:
                ...
                ).cron_folder_ssh_auto_classification()
    ...
    def test_ok_pdf(self, partners=False):
        ...
        with open(os.path.join(self.tmpdir.name, 'test_file.pdf'), 'wb') as f:
            f.write(file)
        code = [Encoded(partner.get_quick_access_code().encode(
            'utf-8')) for partner in partners]
        with patch(
            'odoo.addons.document_quick_access_folder_auto_classification.'
            'models.ir_attachment.decode'
        ) as ptch:
