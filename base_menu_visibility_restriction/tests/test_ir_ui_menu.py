...
class TestIrUiMenuCase(TransactionCase):
    def setUp(self):
        ...
        self.user_admin = self.env['res.users'].browse(SUPERUSER_ID)
        self.group_hide_menu = self.env["res.groups"].create(
            {"name": "Hide menu items custom", "users": [(4, self.user_admin.id)]}
        )
    ...
    def test_ir_ui_menu_admin(self):
        items = self.model_ir_uir_menu.sudo()._visible_menu_ids()
